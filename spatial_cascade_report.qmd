...
## Protection contributes to keeping birds unaffected

![Bird-weighted survival differs by production type and protection level.](assets/spatial_cascade_survival_comparison.svg){#fig-shelter-comparison fig-alt="Share of birds at protected and unprotected farms remaining unaffected by production type and national total, with no culling and with culling"}

```{=html}
<div class="figure-note" role="note"><strong>How to read this figure:</strong>
Each bar shows a survival rate calculated within that specific production and
protection group: birds at unaffected farms in the group are divided by all
birds in the same group, then multiplied by 100. For example, the protected
Eggs bar uses all birds at protected egg farms as its denominator. Larger farms
therefore contribute more to the result. “Total” combines all production types
but still keeps protected and unprotected birds as separate groups. Birds at
culled farms are not counted as surviving. Compare protected and unprotected
bars within each management panel. <strong>What it means:</strong> The culling panel
shows a larger share of birds remaining unaffected. Differences between
protected and unprotected bars also reflect where farms are located, their bird
numbers and where outbreaks begin; the bars do not measure the effect of
protection on its own.</div>
```

The figure compares the share of birds that remain unaffected for each
production type, the national total and each protection level. Every farm is weighted by its bird count,
so a larger farm contributes more to the result than a smaller farm. Without
culling, the share of birds remaining unaffected averaged
**`r round(summary$sheltered_bird_survival_pct, 1)`%** at protected farms,
compared with
**`r round(summary$unsheltered_bird_survival_pct, 1)`%** at farms without protection.
The difference was
**`r round(summary$bird_protection_benefit_percentage_points, 1)` percentage
...
and **`r round(cull_average_bird_survival[[config$scenario_metadata$unsheltered_label]], 1)`%**.

Protection reduces the chance that an outbreak reaches a protected farm. It
therefore contributes to keeping birds unaffected, but it does not guarantee
survival. The final pattern also reflects where protected farms are located,
their bird numbers, where an outbreak starts and which farms it reaches first.
This is why the apparent protection benefit can differ between production
types.

The bars describe the final bird outcomes for the farms currently in each
group; they do not measure the effect of protection on its own. A paired test
that changes only protection, while holding the same farms and outbreaks
fixed, would be needed to isolate that effect.
