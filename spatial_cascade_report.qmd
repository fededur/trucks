...
knitr::kable(capacity_reference[, c("Production type", "Farms", "Total birds",
  "Share of all birds")], row.names = FALSE)
```

The **Grand total** row gives all birds included in the analysis. Farm-level
descriptive statistics are kept in the appendix so they are not confused with
whole-population results.

## Main result: expected total bird losses

This is the main result for communication. It estimates how many birds could
be lost across the **whole farm population**, not at an average farm.

For each farm, the model combines its bird count with how often that farm is
lost across the repeated scenarios. Adding those farm results gives the
expected total for the population. This is the same as averaging the complete
national totals across all runs.
...
The **Total** rows are the national population estimates. Production-type rows
add to each Total. These figures do not mean that part of a farm's birds are
lost in one run: a lost farm contributes all its birds in that run. They are
planning estimates under the selected assumptions, not forecasts.
