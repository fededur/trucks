...
## Protection and culling change how many birds remain unaffected

![Culling increases the expected share of birds remaining unaffected.](assets/spatial_cascade_survival_comparison.svg){#fig-shelter-comparison fig-alt="Expected share of birds at protected and unprotected farms remaining unaffected by production type and national total, with no culling and with culling"}

```{=html}
<div class="figure-note" role="note"><strong>How to read this figure:</strong>
Each bar answers: out of all birds in this group, what share is expected to
remain unaffected? In every modelled outbreak, birds at unaffected farms are
added together and divided by all birds in the same group. The repeated
outbreaks are then combined into the result shown. This is not an average farm:
each farm contributes its actual bird count. A bar at 75% means that 75 out of
every 100 birds in that group are expected to remain unaffected and 25 are
expected to be lost. “Total” combines production types but keeps protected and
unprotected birds separate. Birds at culled farms are counted as lost. Compare
protected and unprotected bars within each management panel.
<strong>What it means:</strong> The culling panel
...
The result is relative to bird numbers, not farm numbers. Without culling, the
expected share of birds lost is
**`r round(100 - summary$sheltered_bird_survival_pct, 1)`% at protected farms**
and **`r round(100 - summary$unsheltered_bird_survival_pct, 1)`% at farms without
protection**. With culling, the expected shares lost fall to
...
respectively. The bars show the matching shares that remain unaffected.
