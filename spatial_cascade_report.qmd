...
## Main result: expected total bird losses
...
complete scenario outcomes varied.
...
The expected share is the expected whole-population loss divided by the birds
in that production type. Percentages in brackets show the 25th–75th percentile
range across complete scenarios with the assumptions held fixed. When a farm
is lost, all birds at that farm are counted as lost in that scenario.</div>
```
...
Across the repeated scenarios, the **expected whole-population loss** was
...
lost in that scenario. The bars are sorted from lower to higher loss within
...
Without culling, the **expected whole-population loss** was
**`r format(round(cull$mean_baseline_total_loss), big.mark = ",")` `r capacity_unit`**.
With culling, the **expected whole-population loss** was
**`r format(round(cull$mean_cull_total_loss), big.mark = ",")` `r capacity_unit`**.
...
of the matched scenarios**. Averaged across complete scenarios, the number of affected farms fell from
