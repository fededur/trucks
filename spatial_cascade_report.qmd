...
national totals across all runs. The results are summarised in
@tbl-expected-population-loss.
...
The Total rows are the national population estimates, and production-type rows
add to each Total. The percentage outside brackets is the expected share lost;
the bracketed 25th–75th percentile range is the middle half of complete
scenario outcomes with assumptions held fixed. A lost farm contributes all its
birds in that run. These are planning estimates, not forecasts.</div>
```
