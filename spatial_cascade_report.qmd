...
This separate table gives one planning estimate for the whole population. For
each farm, the model combines the number of birds with how often that farm was
lost across the repeated scenarios. Those farm results are then added together.
This gives the **expected total birds lost across the simulated scenarios**.
It does not describe an average farm.

```{r}
#| label: population-loss-table
#| tbl-cap: "Expected total bird losses across the farm population"
make_population_rows <- function(scenario, risk_data) {
  rows <- lapply(sort(unique(risk_data$production_type)), function(type) {
    ...
    data.frame(
      ...
      `Expected total birds lost` = estimated_loss,
      `Expected share of birds lost` = 100 * estimated_loss / total_birds,
      check.names = FALSE)
  })
...
This calculation gives the same answer as taking the average of the complete
national bird-loss totals across all runs. It does **not** mean that part of a
farm's birds are lost in one run: within a run, all birds at a lost farm are
counted as lost. It is also not a forecast of a real event.

In short, the first table shows the **range between complete scenarios**. This
table gives the **expected total for the whole population**. Neither table
reports loss for an average farm.
