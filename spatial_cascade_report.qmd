...
```{r}
#| label: type-table
#| tbl-cap: "Bird losses by production type and management response"
make_type_rows <- function(scenario, loss_log, count_log) {
  rows <- lapply(colnames(loss_log), function(type) {
    ...
    average_birds_lost <- mean(loss_log[, type])
    data.frame(Scenario = scenario, `Production type` = type, check.names = FALSE,
      ...
      `Average birds lost` = average_birds_lost,
      `Average birds lost pct` = 100 * average_birds_lost / birds_in_type,
  ...
  rows[[length(rows) + 1L]] <- data.frame(
    ...
    `Average birds lost` = mean(total_loss),
    `Average birds lost pct` = 100 * mean(total_loss) /
      sum(farms$production_yield),
...
display$`Average birds lost out of total` <- paste0(
  format(round(display$`Average birds lost`), big.mark = ","), " of ",
  format(round(display$`Total birds`), big.mark = ","), " (",
  round(display$`Average birds lost pct`, 1), "%)")
display$`Average birds lost` <- NULL
display$`Average birds lost pct` <- NULL
...
display <- display[, c("Scenario", "Production type", "Farms", "Total birds",
  "Average birds lost out of total", "Average farms lost",
...
national farm population within each response scenario. “Average birds lost
out of total” compares the average loss with all birds available in that
production type. The percentage is therefore a share of birds, not a share of
farms.

### Population bird-loss estimates

This separate table builds a population estimate by adding the expected bird
loss from every farm. It does not describe an average farm. The production-type
rows add to the national total within each management response.
