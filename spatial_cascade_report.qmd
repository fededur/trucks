...
![Production lost and remaining across `r format(runs, big.mark = ",")` complete `r days`-day simulations of all `r format(nrow(farms), big.mark = ",")` farms. Each thin vertical bar is one complete scenario and always totals 100%. Scenarios are sorted from lower to higher loss to make the range visible; their left-to-right order is not time and is not the order in which they were run.](assets/spatial_cascade_loss_distribution.svg){#fig-loss-distribution fig-alt="One hundred percent stacked bars showing production lost and remaining in each complete Monte Carlo scenario, ordered from lower to higher loss"}
...
orange means that scenario ended with 80% of national capacity lost. The chart
shows the spread of possible outcomes without turning the number of runs into
an apparent trend.
