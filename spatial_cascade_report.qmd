...
types$expected_loss_pct <- vapply(types$production_type, function(type) {
  type_birds <- sum(farms$production_yield[farms$production_type == type])
  100 * types$mean_total_production_loss[types$production_type == type] /
    type_birds
}, numeric(1))
type_loss_order <- order(types$expected_loss_pct, decreasing = TRUE)
type_loss_comparison <- paste(
  paste0(types$production_type[type_loss_order], ": ",
         round(types$expected_loss_pct[type_loss_order], 1), "%"),
  collapse = "; ")
```
...
- Without culling, the expected share of birds lost by production type is
  **`r type_loss_comparison`**.
