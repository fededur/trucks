...
![Total production capacity lost for each production type across the matched scenarios, split into No culling and Culling panels on the same scale. Each box describes total category loss across the whole population, not the loss of an average farm.](assets/spatial_cascade_loss_by_production_type.svg){#fig-loss-by-type fig-alt="Faceted distribution of total production capacity loss by production type with no culling and with culling"}
...
more capacity will generally have a larger total loss. The panels compare the
