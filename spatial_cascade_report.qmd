...
## National losses vary between possible outbreaks

![Birds lost and remaining across the modelled outbreaks, shown separately with no culling and with culling. Each thin bar is one complete outbreak and totals 100%. Within each panel, outbreaks are sorted from lower to higher loss; the order is not time.](assets/spatial_cascade_loss_distribution.svg){#fig-loss-distribution fig-alt="Percentage of birds lost and remaining in modelled outbreaks with no culling and with culling"}

Across the modelled outbreaks, the **expected national loss** was
**`r format(round(summary$mean_production_loss), big.mark = ",")` `r capacity_unit`**, about
**`r round(mean_loss_share, 1)`%** of all birds. Nine out of ten outbreaks finished between
**`r format(round(summary$loss_p05), big.mark = ",")` and
`r format(round(summary$loss_p95), big.mark = ",")` `r capacity_unit` lost**.

This is a widespread-loss result under the current settings. The expected
population total is useful for planning, while the upper end of the range shows what
...
Each bar is one possible national outcome. Orange shows birds lost and grey
shows birds remaining, so a bar that is 80% orange represents 80% of all birds
lost in that outbreak. The bars are sorted from lower to higher loss within
each panel; their order is not time, and positions across panels are not
the same outbreak in the other panel.

## Managing the outbreak reduces the number of birds lost

The culling response changes the story materially. It accepts an early,
deliberate loss at identified farms to stop a much larger chain from forming.

![Birds lost across the modelled outbreaks, split into No culling and Culling panels on the same scale. Each box shows the middle half of results and its centre line is the median. The culling total includes birds removed directly through culling.](assets/spatial_cascade_cull_comparison.svg){#fig-cull-comparison fig-alt="Birds lost in modelled outbreaks with no culling and with culling"}

Without culling, the **expected national loss** was
**`r format(round(cull$mean_baseline_total_loss), big.mark = ",")` `r capacity_unit`**.
With culling, the **expected national loss** was
**`r format(round(cull$mean_cull_total_loss), big.mark = ",")` `r capacity_unit`**.
...
After counting that direct loss, the response avoided an expected
**`r format(round(cull$mean_net_loss_avoided), big.mark = ",")` `r capacity_unit`**,
...
of the matched modelled outbreaks**. Across all modelled outbreaks, the average number of affected farms fell from
...
## Farm location can outweigh the apparent shelter pattern

![Average share of sheltered and unsheltered farms remaining unaffected at the end of the modelled outbreaks, shown by production type and split into No culling and Culling panels. Both panels use the same percentage scale.](assets/spatial_cascade_survival_comparison.svg){#fig-shelter-comparison fig-alt="Comparison of sheltered and unsheltered farms remaining unaffected by production type, with no culling and with culling"}
...
This compares the two management responses fairly: the farm population and
starting locations are the same, and both versions face the same chance events.

```{r}
#| results: asis
if (protection_difference < 0) {
  cat(paste0(
    "In these results, a slightly smaller share of sheltered farms remains unaffected. ",
  ...
  cat(paste0(
    "A larger share of sheltered farms remains unaffected in these results. Some of ",
...
## Early action changes the national outcome

Under the current settings, the main message is that loss is widespread in
most modelled outbreaks. The range across runs matters because it shows that the final
...
The most useful result for communication is the expected total loss across the
whole population. The production-type results explain who carries that loss,
and the map shows where exposure repeatedly appears. Together they suggest
that early culling can sharply reduce national loss under the selected
conditions, while still leaving meaningful uncertainty between possible
events. This is planning evidence rather than a forecast.
